...
checkboxes.forEach(function(checkbox) {
  checkbox.addEventListener('change', function() {
    // Build an object with the selected filters
    const brands = [];
    const tags = [];
    checkboxes.forEach(function(checkbox) {
      if (checkbox.checked && checkbox.getAttribute("data-filter")=="brands") {
        brands.push(checkbox.value);
      }else if(checkbox.checked && checkbox.getAttribute("data-filter")=="tags"){tags.push(checkbox.value);}
    });


    // Send an AJAX request to the server to fetch the filtered results
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'filter.php');
    xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhr.onload = function() {
      if (xhr.status === 200) {
        // Receive the JSON data and update the product list
        ...
        products.forEach(function(product) {
          ...
          const prodInnerDiv=document.createElement('div');
          prodInnerDiv.classList.add("tf-product")
          ...
          a.setAttribute("href",`item-detail.php?id=${product.id}`)
          ...
          h6.appendChild(span)
          prodInnerDiv.appendChild(img)
          prodInnerDiv.appendChild(h6)
          productDiv.appendChild(prodInnerDiv);
    ...
    xhr.send('brands=' + JSON.stringify(brands)+'&tags='+JSON.stringify(tags));
  });
});